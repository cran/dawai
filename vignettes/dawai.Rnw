%\VignetteIndexEntry{dawai: An R Package for Discriminant Analysis With Additional Information}
...
<<biologicalApplication, eval=FALSE, results=hide>>=
library(survival)
data("pbc")
...
<<patternRecognitionApplication, eval=FALSE, results=hide>>=
data("Vehicle2")
data <- Vehicle2[, 1:3]
